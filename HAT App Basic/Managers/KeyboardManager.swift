...
class KeyboardManager: NSObject {
    ...
    public func dismissKeyboard() {
        ...
        if self.hatUIViewScrollView == nil {
            ...
            self.hatUIViewScrollView?.contentInset = .zero
            self.hatUIViewScrollView?.scrollIndicatorInsets = .zero
            self.view.endEditing(true)
        }
    }
    ...
    public func keyboardWillShow(sender: NSNotification) {
        ...
        if self.hatUIViewScrollView == nil {
            ...
            if let activeField: UITextField = self.activeField,
                (!aRect.contains(activeField.frame.origin)) {
    ...
    func keyboardWillHide(sender: NSNotification) {
        ...
        guard let keyboardSize: CGSize = (sender.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.size,
            self.isKeyboardShown else { return }
    ...
    // MARK: - Set up keyboard handling
    
    /**
     Sets up the keyboard handling for the specific view and scrollView
     
     - parameter view: The view that has to adapt in order to host the keyboard
     - parameter scrollView: The scrollView that needs to scroll with the keyboard
     */
    func setUpKeyboardHandlingFor(_ view: UIView, scrollView: UIScrollView) {
